...
int getMaior(int n) {
    ...
    for (int i = 0; i < n; i++) {
        if(maior.upvotes < games[i].upvotes){
            maior = games[i];
        }
    }
    return (int)maior.upvotes ;
}
...
void countingsort(int n) {
    FILE *registro = fopen("matrı́cula_countingsort.txt.","w");
    ...
    Game ordenado[n];
        ...
        //Inicializar cada posicao do games de contagem 
    for (int i = 0; i < tamCount; count[i] = 0, i++);

    //Agora, o count[i] contem o numero de elemento iguais a i
    for (int i = 0; i < n; count[(int)games[i].upvotes]++, i++);

    //Agora, o count[i] contem o numero de elemento menores ou iguais a i
    for(int i = 1; i < tamCount; count[i] += count[i-1], i++);

    //Ordenando
    for(int i = n-1; i >= 0; ordenado[count[(int)games[i].upvotes]-1] = games[i], count[(int)games[i].upvotes]--, i--){
        movimentacao++;
    }

    //Copiando para o games original
    for(int i = 0; i < n; games[i] = ordenado[i], i++){
        movimentacao++;
    }
